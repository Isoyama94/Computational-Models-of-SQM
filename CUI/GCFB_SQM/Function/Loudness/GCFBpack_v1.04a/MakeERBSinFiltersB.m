...
gain = abs(-(exp((B + 2*i*cf*pi)*T)*T.*  ...
    (-2*exp(4*i*cf*pi*T)*T + ...
    (1+i)*(1 - i*exp(4*i*cf*pi*T))*T./exp(B*T)).* ...
    (-2*exp(4*i*cf*pi*T)*T + ...
    (1+i)*(-i + exp(4*i*cf*pi*T))*T./exp(B*T)).*  ...
    (-2*exp(4*i*cf*pi*T)*T +  ...
    (1 + exp(4*i*cf*pi*T))*T./exp(B*T)).*sin(2*cf*pi*T))./ ...
    (2*cf.*(-1 + exp(B*T) - exp(2*(B + 2*i*cf*pi)*T) + ...
    exp((B + 4*i*cf*pi)*T)).*  ...
    (-2./exp(2*B*T) - 2*exp(4*i*cf*pi*T) +  ...
    2*(1 + exp(4*i*cf*pi*T))./exp(B*T)).^3* pi));
