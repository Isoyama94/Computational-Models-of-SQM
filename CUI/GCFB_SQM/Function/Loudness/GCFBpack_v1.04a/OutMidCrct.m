...
disp(['*** Outer/Middle Ear Transfer Function ( ' upper(StrCrct) ' Correction ) ***']);
if Npnts <= 0,
  disp('No interpolation. Output original table.');
  frqNpnts = frqTbl; 
  CrctdB = CrctTbl; 
else
  ...
  if strcmp(upper(StrCrct(1:2)), 'NO'),
    ...
    disp('Spline interpolated value in equal frequency spacing.');
    CrctdB = spline(frqTbl,CrctTbl,frqNpnts);	
  end;
end;

if SwPlot == 1, plot(frqTbl,CrctTbl,frqNpnts,CrctdB,'o'); end;
