...
%         GTout     : Output of gammatone filterbank
%
...
%
function [Nt,N,Npk,Excitation,GTout] = GTLoudnessModel(St,Fs0,SPL,Setting)
%% Initial setup
[m,n] = size(St);
St = reshape(St,[1,m*n]);
...
[QL,E0,TQ_power,G_power,alph,A_data] = GTLoudnessParameters(Setting);

%% Sound pressure level
out = GTAdaptLevel(St,SPL,Setting);

%% Outer and middle ear correction
[yt] = OuterMiddleEarFilter(out,Fs0,Setting);

%% Auditory filtering
[GTout,~]=IIRGammaToneFB(yt',Fs0,Setting.ERBNnumber,1.019,'Normal','OFF',Setting);

%% Calcuration of excitation
[Excitation,~] = CalculationOfExcitation(GTout,Fs0,E0,Setting);
