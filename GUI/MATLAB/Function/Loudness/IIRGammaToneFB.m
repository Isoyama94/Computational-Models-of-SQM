...
switch swIIRmode
    ...
    case {'Slaney'}  % Slaney's implimentation
        ...
        gain = abs( (-2*exp(1j*4*cf*pi*T)*T + 2*exp(-(B*T) + 1j*2*pi*cf*T).*T.* ...
            (cos(2*pi*cf*T) - sqrt(3 - 2^(3/2))* sin(2*pi*cf*T))) .* ... % Hb1
            (-2*exp(1j*4*cf*pi*T)*T + 2*exp(-(B*T) + 1j*2*cf*pi*T).*T.* ...
            ...
            (-2./exp(2*B*T) - 2*exp(1j*4*cf*pi*T) + ...
            2*(1 + exp(1j*4*cf*pi*T))./exp(B*T)).^4 ); % Han
...
switch swIIRmode
    ...
    case {'Normal', 'OZGT', 'APGT'}
        ...
        gain1=abs( (bz1(:,1).*exp(1j*4*pi*cf*T)+bz1(:,2).*exp(1j*2*pi*cf*T)) ./ ...
            (ap1(:,1).*exp(1j*4*pi*cf*T)+ap1(:,2).*exp(1j*2*pi*cf*T)+ap1(:,3)) );
        gain2=abs( (bz2(:,1).*exp(1j*4*pi*cf*T)+bz2(:,2).*exp(1j*2*pi*cf*T)) ./ ...
            (ap2(:,1).*exp(1j*4*pi*cf*T)+ap2(:,2).*exp(1j*2*pi*cf*T)+ap2(:,3)) );
        gain3=abs( (bz3(:,1).*exp(1j*4*pi*cf*T)+bz3(:,2).*exp(1j*2*pi*cf*T)) ./ ...
            (ap3(:,1).*exp(1j*4*pi*cf*T)+ap3(:,2).*exp(1j*2*pi*cf*T)+ap3(:,3)) );
        gain4=abs( (bz4(:,1).*exp(1j*4*pi*cf*T)+bz4(:,2).*exp(1j*2*pi*cf*T)) ./ ...
            (ap4(:,1).*exp(1j*4*pi*cf*T)+ap4(:,2).*exp(1j*2*pi*cf*T)+ap4(:,3)) );
